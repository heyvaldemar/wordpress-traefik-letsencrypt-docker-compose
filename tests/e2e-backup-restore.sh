#!/bin/bash
# End-to-end tests for the wordpress-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-wordpress}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-wordpress-traefik-letsencrypt-docker-compose.yml}"
...
: "${WORDPRESS_DB_PASSWORD:?set in .env}"
...
test_restore_roundtrip() {
  ...
  echo "  restoring the baseline with ./wordpress-restore-database.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./wordpress-restore-database.sh "$(basename "$baseline")" \
...
test_data_restore_roundtrip() {
  ...
  echo "  restoring $(basename "$newest") with ./wordpress-restore-application-data.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./wordpress-restore-application-data.sh "$(basename "$newest")" \
    || { fail "the shipped restore script failed"; return 1; }
